...
void credx_buffer_free(struct ByteBuffer buffer);

ErrorCode credx_create_credential(
    ...
    FfiStrList attr_raw_values, FfiStrList attr_enc_values,
    const struct FfiCredRevInfo *revocation, ObjectHandle *cred_p,
    ObjectHandle *rev_reg_p, ObjectHandle *rev_delta_p);

ErrorCode credx_create_credential_definition(
    FfiStr origin_did, ObjectHandle schema, FfiStr tag, FfiStr signature_type,
    int8_t support_revocation, ObjectHandle *cred_def_p,
    ObjectHandle *cred_def_pvt_p, ObjectHandle *key_proof_p);

ErrorCode credx_create_credential_offer(FfiStr schema_id, ObjectHandle cred_def,
                                        ObjectHandle key_proof,
                                        ObjectHandle *cred_offer_p);

ErrorCode credx_create_credential_request(
...
ErrorCode credx_create_or_update_revocation_state(
    ObjectHandle rev_reg_def, ObjectHandle rev_reg_delta, int64_t rev_reg_index,
    int64_t timestamp, FfiStr tails_path, ObjectHandle rev_state,
    ObjectHandle *rev_state_p);

ErrorCode credx_create_presentation(
    ...
    struct FfiList_ObjectHandle cred_defs, ObjectHandle *presentation_p);

ErrorCode credx_create_revocation_registry(
    FfiStr origin_did, ObjectHandle cred_def, FfiStr tag, FfiStr rev_reg_type,
    FfiStr issuance_type, int64_t max_cred_num, FfiStr tails_dir_path,
    ObjectHandle *reg_def_p, ObjectHandle *reg_def_private_p,
    ObjectHandle *reg_entry_p, ObjectHandle *reg_init_delta_p);

ErrorCode credx_create_schema(FfiStr origin_did, FfiStr schema_name,
                              FfiStr schema_version, FfiStrList attr_names,
                              int64_t seq_no, ObjectHandle *result_p);

ErrorCode credx_credential_definition_get_attribute(ObjectHandle handle,
                                                    FfiStr name,
                                                    const char **result_p);

ErrorCode credx_credential_get_attribute(ObjectHandle handle, FfiStr name,
                                         const char **result_p);

ErrorCode credx_encode_credential_attributes(FfiStrList attr_raw_values,
                                             const char **result_p);
...
ErrorCode credx_object_get_json(ObjectHandle handle,
                                struct ByteBuffer *result_p);

ErrorCode credx_object_get_type_name(ObjectHandle handle,
                                     const char **result_p);

ErrorCode
credx_process_credential(ObjectHandle cred, ObjectHandle cred_req_metadata,
                         ObjectHandle master_secret, ObjectHandle cred_def,
                         ObjectHandle rev_reg_def, ObjectHandle *cred_p);

ErrorCode credx_revocation_registry_definition_get_attribute(
    ObjectHandle handle, FfiStr name, const char **result_p);

ErrorCode credx_revoke_credential(ObjectHandle rev_reg_def,
                                  ObjectHandle rev_reg, int64_t cred_rev_idx,
                                  FfiStr tails_path, ObjectHandle *rev_reg_p,
                                  ObjectHandle *rev_reg_delta_p);

ErrorCode credx_schema_get_attribute(ObjectHandle handle, FfiStr name,
                                     const char **result_p);

ErrorCode credx_set_default_logger(void);

ErrorCode credx_update_revocation_registry(
    ObjectHandle rev_reg_def, ObjectHandle rev_reg, struct FfiList_i64 issued,
    struct FfiList_i64 revoked, FfiStr tails_path, ObjectHandle *rev_reg_p,
    ObjectHandle *rev_reg_delta_p);

ErrorCode credx_verify_presentation(
    ObjectHandle presentation, ObjectHandle pres_req,
    struct FfiList_ObjectHandle schemas, struct FfiList_ObjectHandle cred_defs,
    struct FfiList_ObjectHandle rev_reg_defs,
    struct FfiList_FfiRevocationEntry rev_reg_entries, int8_t *result_p);

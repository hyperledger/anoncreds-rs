...
use indy_data_types::{
    anoncreds::{
        ...
        nonce::Nonce,
        pres_request::{
            AttributeInfo, NonRevocedInterval, PredicateInfo, PresentationRequest,
            PresentationRequestPayload,
        },
        presentation::{Identifier, Presentation, RequestedProof, RevealedAttributeInfo},
...
impl Verifier {
    ...
    fn _received_revealed_attrs(proof: &Presentation) -> Result<HashMap<String, Identifier>> {
    ...
    fn _received_unrevealed_attrs(proof: &Presentation) -> Result<HashMap<String, Identifier>> {
    ...
    fn _received_predicates(proof: &Presentation) -> Result<HashMap<String, Identifier>> {
    ...
    fn _received_self_attested_attrs(proof: &Presentation) -> HashSet<String> {
        proof
            .requested_proof
            .self_attested_attrs
            .keys()
    ...
    fn _get_proof_identifier(proof: &Presentation, index: u32) -> Result<Identifier> {
        proof
            .identifiers
            .get(index as usize)
            .cloned()
    ...
    fn _verify_requested_restrictions(
            ...
            if let Some(ref query) = info.restrictions {
                ...
                let name_value_map: HashMap<String, Option<&str>> = if let Some(name) = info.name {
                    ...
                    let attrs = requested_proof
                        ...
                        .get(&referent)
                        .ok_or_else(|| {
                            err_msg!("Proof does not have referent from proof request")
                        })?;
                    for name in names {
                ...
                Self::_do_process_operator(&name_value_map, &query, &filter).map_err(err_map!(
        ...
        for (referent, info) in proof_req.requested_predicates.iter() {
            if let Some(ref query) = info.restrictions {
                ...
                Self::_process_operator(&info.name, &query, &filter, None).map_err(err_map!(
    ...
    fn _precess_filed(filed: &str, filter_value: &str, tag_value: &str) -> Result<()> {

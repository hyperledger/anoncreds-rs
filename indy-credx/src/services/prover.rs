...
use indy_data_types::{
    anoncreds::{
        ...
        pres_request::{
            PresentationRequest, PresentationRequestPayload, RequestedAttributeInfo,
            RequestedPredicateInfo,
        },
        presentation::{
            AttributeValue, Identifier, Presentation, RequestedProof, RevealedAttributeGroupInfo,
...
use indy_utils::{did::DidValue, invalid, Qualifiable, Validatable, ValidationError};
...
pub struct RevocationState {
    witness: Witness,
    rev_reg: ursa::cl::RevocationRegistry,
    timestamp: u64,
}
        ...
        for (cred_key, (req_attrs_for_cred, req_predicates_for_cred)) in credentials_for_proving {
            ...
            let rev_state = if let Some(timestamp) = cred_key.timestamp {
                ...
                let cred_rev_states = rev_states
                    ...
                    .ok_or_else(|| {
                        err_msg!("Revocation State not provided for ID: {}", rev_reg_id)
                    })?;
